...
export const subscriptionsApi = {
    follow: async (userId: number): Promise<{
        message: string;
    }> => {
        const response = await apiClient.post<{
            message: string;
        }>(`/subscriptions/follow/${userId}`);
        return response.data;
    },
    unfollow: async (userId: number): Promise<{
        message: string;
    }> => {
        const response = await apiClient.delete<{
            message: string;
        }>(`/subscriptions/unfollow/${userId}`);
        return response.data;
    },
    ...
    isFollowing: async (userId: number): Promise<{
        isFollowing: boolean;
    }> => {
        const response = await apiClient.get<{
            isFollowing: boolean;
        }>(`/subscriptions/is-following/${userId}`);
        return response.data;
    },
};
