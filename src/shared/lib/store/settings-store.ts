...
const MAP_STYLE_KEYS = new Set(Object.keys(MAP_STYLES) as MapStyleKey[]);
...
function normalizeMapSettingsPayload(settings: Pick<MapSettings, "availableMapStyles" | "defaultMapStyle"> & {
    pointCardInitialView?: PointCardInitialView;
}): MapSettings {
    ...
    const finalAvailable = available.length > 0 ? available : [...DEFAULT_MAP_STYLES];
...
export const useSettingsStore = create<SettingsState>((set, get) => ({
    ...
    setAvailableMapStyles: async (styles) => {
        ...
        set({ isLoading: true });
        try {
            const updated = normalizeMapSettingsPayload(await settingsApi.updateMapSettings({
