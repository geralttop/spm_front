...
export const PointCard = React.memo(function PointCard({ point, showAuthor = true, fromContainerId, showMapLink = true, onFavoriteChange, onPointUpdate, }: PointCardProps) {
    ...
    const { isFavorite, isLoading, toggleFavorite } = useFavoriteStatus(point.id, onFavoriteChange);
    ...
    const sharePath = useMemo(() => fromContainerId
        ? `/points/${point.id}?fromContainer=${encodeURIComponent(fromContainerId)}`
        : `/points/${point.id}`, [fromContainerId, point.id]);
    ...
    return (<div className="bg-surface border border-border rounded-xl p-3 sm:p-6 hover:shadow-md transition-shadow">
      {showAuthor && (<div className="mb-3 sm:mb-4 flex items-center gap-2.5 sm:gap-3">
          <Link href={userProfilePath(point.author.username)} className="flex min-w-0 flex-1 items-center gap-2.5 sm:gap-3 rounded-xl -m-1 p-1 transition-colors hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background" aria-label={t("search.viewProfile")}>
            <div className="h-9 w-9 sm:h-10 sm:w-10 flex-shrink-0 overflow-hidden rounded-full border-2 border-border bg-primary/10">
              {point.author.avatar ? (<img src={`${process.env.NEXT_PUBLIC_API_URL}${point.author.avatar}`} alt="" className="h-full w-full object-cover"/>) : (<div className="flex h-full w-full items-center justify-center">
      ...
      {!showAuthor && (<div className="mb-3 sm:mb-4 flex flex-wrap items-center justify-between gap-x-2 gap-y-2">
      ...
      <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-border">
        <button type="button" onClick={() => setShowComments(!showComments)} className="flex items-center gap-2 text-xs sm:text-sm text-primary hover:text-primary/80 transition-colors font-medium touch-target min-h-[44px] -mx-1 px-1 rounded-lg active:bg-primary/10">
          <MessageCircle className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0"/>
          <span>
            {showComments ? t("comments.hide") : t("comments.show")}
          </span>
          {point.commentsCount !== undefined && point.commentsCount > 0 && (<span className="px-2 py-0.5 bg-primary/10 text-primary rounded-full text-xs font-semibold">
              {point.commentsCount}
            </span>)}
        </button>
      ...
      <ReportModal isOpen={showReportModal} onClose={() => setShowReportModal(false)} type="point" targetId={point.id} targetName={point.name} onSuccess={handleReportSuccess}/>

      <EditPointModal isOpen={showEditModal} onClose={() => setShowEditModal(false)} point={point} onSuccess={handleEditSuccess}/>
    </div>);
});
