...
export function PointCardSkeleton({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "rounded-xl border border-border bg-surface p-3 sm:p-6",
        className
      )}
      aria-hidden
    >
      ...
      <div className="-mx-3 overflow-hidden rounded-none border border-border bg-muted/20 sm:mx-0 sm:rounded-lg">
        <div
          className="w-full bg-muted/40 dark:bg-muted/30"
...
type PointCardSkeletonListProps = {
  count?: number;
  /** Aria-label для области (i18n), например t('feed.loading') */
  ariaLabel: string;
  className?: string;
};

export function PointCardSkeletonList({
  count = DEFAULT_COUNT,
  ariaLabel,
  className,
}: PointCardSkeletonListProps) {
  return (
    <div
      role="status"
      aria-busy="true"
      aria-label={ariaLabel}
      className={cn("-mx-3 space-y-4 sm:mx-0 sm:space-y-6", className)}
    >
