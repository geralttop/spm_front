...
function MapMarker({ longitude, latitude, children, onClick, onMouseEnter, onMouseLeave, onDragStart, onDrag, onDragEnd, draggable = false, ...markerOptions }: MapMarkerProps) {
    ...
    return (<MarkerContext.Provider value={{ marker, map }}>
      {children}
...
function DefaultMarkerIcon() {
    return (<div className="relative h-4 w-4 rounded-full border-2 border-background bg-primary shadow-lg"/>);
}
...
function MarkerPopup({ children, className, closeButton = false, onOpenChange, initialOpen = false, ...popupOptions }: MarkerPopupProps) {
...
function MarkerTooltip({ children, className, ...popupOptions }: MarkerTooltipProps) {
    ...
    return createPortal(<div className={cn("rounded-md bg-foreground px-2 py-1 text-xs text-background shadow-md animate-in fade-in-0 zoom-in-95", className)}>
      {children}
    </div>, container);
}
...
function MarkerLabel({ children, className, position = "top", }: MarkerLabelProps) {
    ...
    return (<div className={cn("absolute left-1/2 -translate-x-1/2 whitespace-nowrap", "text-[10px] font-medium text-foreground", positionClasses[position], className)}>
      {children}
    </div>);
}
export { MapMarker, MarkerContent, MarkerPopup, MarkerTooltip, MarkerLabel, DefaultMarkerIcon, };
