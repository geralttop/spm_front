...
export function MessengerComposer({ value, onChange, onSend, placeholder, sendAriaLabel, disabled = false, sending = false, maxLength = 2000, className, }: MessengerComposerProps) {
    ...
    return (<div className={cn("flex w-full min-w-0 items-end gap-2 sm:gap-3", className)}>
      <div className="flex min-h-11 min-w-0 flex-1 items-end rounded-[1.375rem] border border-border bg-card px-3 py-2 shadow-sm">
        <TextareaAutosize value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} maxLength={maxLength} minRows={1} maxRows={7} className={cn("w-full resize-none bg-transparent text-sm leading-5 text-text-main", "placeholder:text-text-muted focus:outline-none", "sm:text-[15px] sm:leading-5")} onKeyDown={(e) => {
