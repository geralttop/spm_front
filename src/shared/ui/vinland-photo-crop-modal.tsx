...
import {
  VINLAND_CROP_OUTPUT_HEIGHT,
  VINLAND_CROP_OUTPUT_WIDTH,
  VINLAND_MEDIA_RATIO_W,
  VINLAND_MEDIA_RATIO_H,
} from "@/shared/lib/vinland-media-aspect";

interface VinlandPhotoCropModalProps {
  imageSrc: string;
  onCropComplete: (croppedImage: Blob) => void;
  onClose: () => void;
}

export function VinlandPhotoCropModal({
  imageSrc,
  onCropComplete,
  onClose,
}: VinlandPhotoCropModalProps) {
  ...
  const createCroppedImage = async () => {
    ...
    canvas.width = VINLAND_CROP_OUTPUT_WIDTH;
    canvas.height = VINLAND_CROP_OUTPUT_HEIGHT;
  ...
  const aspect = VINLAND_MEDIA_RATIO_W / VINLAND_MEDIA_RATIO_H;
    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/80 p-0 sm:items-center sm:p-4">
      <div className="relative flex max-h-[95vh] w-full max-w-2xl flex-col overflow-hidden rounded-t-2xl bg-card shadow-xl sm:max-h-[90vh] sm:rounded-xl">
        <div className="flex shrink-0 items-center justify-between border-b border-border p-4">
          <h3 className="text-lg font-semibold text-text-main">{t("vinland.cropTitle")}</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex shrink-0 gap-3 border-t border-border p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
          <Button
            onClick={handleSave}
