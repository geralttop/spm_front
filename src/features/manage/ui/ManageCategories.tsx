...
export function ManageCategories() {
    ...
    const virtualCategoryPointsCount = useMemo(() => countPointsInVirtualCategory(points, systemCategoryId), [points, systemCategoryId]);
    const virtualCategoryPoints = useMemo(() => listPointsInVirtualCategory(points, systemCategoryId), [points, systemCategoryId]);
    ...
    const handleDropOnCategory = async (categoryId: number | null) => {
        if (!draggedPoint)
            return;
        const pointEffectivelyNoCategory = !draggedPoint.category ||
            (systemCategoryId != null && draggedPoint.category.id === systemCategoryId);
        if (categoryId === null && pointEffectivelyNoCategory)
            return;
        if (categoryId !== null && draggedPoint.category?.id === categoryId)
            return;
        setCategoriesLoading(true);
        try {
    ...
    return (<div>
      <div className="flex flex-col xs:flex-row justify-between xs:items-center gap-2 mb-4 sm:mb-6">
        ...
        {!showCategoryForm && !editingCategory && (<button onClick={() => setShowCategoryForm(true)} className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors text-sm sm:text-base self-start xs:self-auto">
      ...
      {(showCategoryForm || editingCategory) && (<CategoryForm editingCategory={editingCategory} onSuccess={handleFormSuccess} onCancel={handleFormCancel}/>)}

      {categories.length === 0 ? (<p className="text-center py-8 text-text-muted">{t('manage.manageCategories.noCategories')}</p>) : (<div className="space-y-3">
          
          <div className={`bg-surface rounded-lg border-2 transition-colors ${draggedPoint &&
                draggedPoint.category !== null &&
                (systemCategoryId == null || draggedPoint.category?.id !== systemCategoryId)
                ? 'border-primary/40 bg-primary/10'
                : 'border-border'}`} onDragOver={handleDragOver} onDrop={() => handleDropOnCategory(null)}>
            <div className="flex items-center justify-between p-3 sm:p-4">
              <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                <button type="button" onClick={() => toggleCategoryExpand(MANAGE_VIRTUAL_CATEGORY_EXPAND_ID)} className="p-1 hover:bg-accent rounded transition-colors shrink-0">
                  {isVirtualCategoryExpanded ? (<ChevronDown className="h-4 w-4"/>) : (<ChevronRight className="h-4 w-4"/>)}
                </button>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {virtualCategoryPointsCount > 0 && (<p className="text-xs text-text-muted mt-0.5 sm:mt-1">
            ...
            {isVirtualCategoryExpanded && virtualCategoryPoints.length > 0 && (<div className="px-3 sm:px-4 pb-3 sm:pb-4 space-y-2">
                {virtualCategoryPoints.map((point) => (<div key={point.id} draggable onDragStart={() => handleDragStart(point)} onDragEnd={handleDragEnd} className={`flex items-center gap-2 p-2 sm:p-3 bg-background rounded border transition-colors ${draggedPoint?.id === point.id
            ...
            {isVirtualCategoryExpanded && virtualCategoryPoints.length === 0 && (<div className="px-4 pb-4">
          ...
          {displayCategories.map((category) => {
                ...
                return (<div key={category.id} className={`bg-surface rounded-lg border-2 transition-colors ${draggedPoint && draggedPoint.category?.id !== category.id ? 'border-primary/40 bg-primary/10' : 'border-border'}`} onDragOver={handleDragOver} onDrop={() => handleDropOnCategory(category.id)}>
                <div className="flex items-center justify-between p-3 sm:p-4">
                  <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                    <button onClick={() => toggleCategoryExpand(category.id)} className="p-1 hover:bg-accent rounded transition-colors shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {categoryPoints.length > 0 && (<p className="text-xs text-text-muted mt-0.5 sm:mt-1">{categoryPoints.length} {t('manage.manageCategories.points')}</p>)}
                    </div>
                  </div>
                ...
                {isExpanded && categoryPoints.length > 0 && (<div className="px-3 sm:px-4 pb-3 sm:pb-4 space-y-2">
                    {categoryPoints.map((point) => (<div key={point.id} draggable onDragStart={() => handleDragStart(point)} onDragEnd={handleDragEnd} className={`flex items-center gap-2 p-2 sm:p-3 bg-background rounded border transition-colors ${draggedPoint?.id === point.id ? 'border-primary opacity-50' : 'border-border hover:border-primary cursor-move'}`}>
                        <GripVertical className="h-4 w-4 text-text-muted shrink-0 hidden sm:block"/>
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-xs sm:text-sm text-text-main truncate">{point.name}</p>
                          {point.description && <p className="text-xs text-text-muted truncate">{point.description}</p>}
                ...
                {isExpanded && categoryPoints.length === 0 && (<div className="px-4 pb-4">
