...
export function ManageContainers() {
    ...
    const virtualContainerPointsCount = useMemo(() => countPointsInVirtualContainer(points, systemContainerId), [points, systemContainerId]);
    const virtualContainerPoints = useMemo(() => listPointsInVirtualContainer(points, systemContainerId), [points, systemContainerId]);
    ...
    const handleDropOnContainer = async (containerId: string | null) => {
        if (!draggedPoint)
            return;
        const pointEffectivelyNoContainer = !draggedPoint.container ||
            (systemContainerId != null && draggedPoint.container.id === systemContainerId);
        if (containerId === null && pointEffectivelyNoContainer)
            return;
        if (containerId !== null && draggedPoint.container?.id === containerId)
            return;
        setContainersLoading(true);
        try {
    ...
    return (<div>
      <div className="flex flex-col xs:flex-row justify-between xs:items-center gap-2 mb-4 sm:mb-6">
        ...
        {!showContainerForm && !editingContainer && (<button onClick={() => setShowContainerForm(true)} className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors text-sm sm:text-base self-start xs:self-auto">
      ...
      {(showContainerForm || editingContainer) && (<ContainerForm editingContainer={editingContainer} onSuccess={handleFormSuccess} onCancel={handleFormCancel}/>)}

      {containers.length === 0 ? (<p className="text-center py-8 text-text-muted">{t('manage.manageContainers.noContainers')}</p>) : (<div className="space-y-3">
          
          <div className={`bg-surface rounded-lg border-2 transition-colors ${draggedPoint &&
                draggedPoint.container !== null &&
                (systemContainerId == null || draggedPoint.container?.id !== systemContainerId)
                ? 'border-primary/40 bg-primary/10'
                : 'border-border'}`} onDragOver={handleDragOver} onDrop={() => handleDropOnContainer(null)}>
            <div className="flex items-center justify-between p-3 sm:p-4">
              <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                <button type="button" onClick={() => toggleContainerExpand(MANAGE_VIRTUAL_CONTAINER_EXPAND_ID)} className="p-1 hover:bg-accent rounded transition-colors shrink-0">
                  {isVirtualContainerExpanded ? (<ChevronDown className="h-4 w-4"/>) : (<ChevronRight className="h-4 w-4"/>)}
                </button>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {virtualContainerPointsCount > 0 && (<p className="text-xs text-primary mt-0.5 sm:mt-1">
            ...
            {isVirtualContainerExpanded && virtualContainerPoints.length > 0 && (<div className="px-3 sm:px-4 pb-3 sm:pb-4 space-y-2">
                {virtualContainerPoints.map((point) => (<div key={point.id} draggable onDragStart={() => handleDragStart(point)} onDragEnd={handleDragEnd} className={`flex items-center gap-2 p-2 sm:p-3 bg-background rounded border transition-colors ${draggedPoint?.id === point.id
            ...
            {isVirtualContainerExpanded && virtualContainerPoints.length === 0 && (<div className="px-4 pb-4">
          ...
          {displayContainers.map((container) => {
                ...
                return (<div key={container.id} className={`bg-surface rounded-lg border-2 transition-colors ${draggedPoint && draggedPoint.container?.id !== container.id ? 'border-primary/40 bg-primary/10' : 'border-border'}`} onDragOver={handleDragOver} onDrop={() => handleDropOnContainer(container.id)}>
                <div className="flex items-center justify-between p-3 sm:p-4">
                  <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                    <button onClick={() => toggleContainerExpand(container.id)} className="p-1 hover:bg-accent rounded transition-colors shrink-0">
                      {isExpanded ? <ChevronDown className="h-4 w-4"/> : <ChevronRight className="h-4 w-4"/>}
                    </button>
                    <span className="h-4 w-4 shrink-0 rounded-full border border-border" style={{ backgroundColor: container.color }} title={container.color}/>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-sm sm:text-base text-text-main truncate">{container.title}</h3>
                      {container.description && (<p className="text-xs sm:text-sm text-text-muted mt-0.5 sm:mt-1 line-clamp-1 sm:line-clamp-none">{container.description}</p>)}
                      {containerPoints.length > 0 && (<p className="text-xs text-primary mt-0.5 sm:mt-1">📍 {containerPoints.length} {t('manage.manageContainers.points')}</p>)}
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-1 sm:flex-row sm:items-center sm:gap-2 shrink-0">
                    ...
                    <button onClick={() => handleDeleteContainer(container.id)} className="p-1.5 sm:p-2 text-destructive hover:bg-destructive/10 rounded-lg transition-colors">
                ...
                {isExpanded && containerPoints.length > 0 && (<div className="px-3 sm:px-4 pb-3 sm:pb-4 space-y-2">
                    {containerPoints.map((point) => (<div key={point.id} draggable onDragStart={() => handleDragStart(point)} onDragEnd={handleDragEnd} className={`flex items-center gap-2 p-2 sm:p-3 bg-background rounded border transition-colors ${draggedPoint?.id === point.id ? 'border-primary opacity-50' : 'border-border hover:border-primary cursor-move'}`}>
                        <GripVertical className="h-4 w-4 text-text-muted shrink-0 hidden sm:block"/>
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-xs sm:text-sm text-text-main truncate">{point.name}</p>
                          {point.description && <p className="text-xs text-text-muted truncate">{point.description}</p>}
                ...
                {isExpanded && containerPoints.length === 0 && (<div className="px-4 pb-4">
