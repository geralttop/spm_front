        ...
        <>
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <h2 className="truncate text-xl font-bold text-text-main sm:text-2xl">
                {t('favorites.title')}
              </h2>
            </div>
            <button
          ...
          <PointCardSkeletonList ariaLabel={t('feed.loading')} />
        </>
      ) : (
        <>
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {favorites.length === 0 ? (
            <div className="py-12 text-center sm:py-16">
              ...
              <h3 className="mb-2 text-lg font-semibold text-text-main sm:text-xl">
                {t('favorites.empty')}
              </h3>
              <p className="mx-auto mb-6 max-w-md text-sm text-text-muted sm:text-base">
              ...
              <div className="mb-4 sm:mb-6">
                <div className="rounded-lg border border-border bg-surface p-2.5 sm:p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                    <span className="shrink-0 text-sm font-medium text-text-main">
                      {t('favorites.sortBy')}
                    </span>
                    <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 [-ms-overflow-style:none] [scrollbar-width:none] sm:mx-0 sm:flex-wrap sm:overflow-visible sm:pb-0 [&::-webkit-scrollbar]:hidden">
                      ...
                      >
                        ...
                        <span className="hidden sm:inline">
                          {t('favorites.sortByDate')}
                        </span>
                        <span className="sm:hidden">
                          {t('favorites.sortByDateShort')}
                        </span>
                      </button>
                      <button
                        type="button"
                ...
                {sortedFavorites.map((favorite) => (
                  <div key={favorite.id} className="space-y-1">
                    ...
                    <p className="px-3 text-xs text-text-muted sm:px-0 sm:text-sm">
                      ...
                      <span className="sm:hidden">
                        {t('favorites.addedAt')}{' '}
                        {formatAddedDateShort(favorite.addedAt)}
                      </span>
                    </p>
                  </div>
                ))}
