...
export function ProfileHeader({ username, email, avatar, role, createdPointsCount, isEditing, onEdit }: ProfileHeaderProps) {
    ...
    const avatarUrl = avatar
        ...
        : null;
    return (<div className="rounded-lg border border-border bg-card p-4 sm:p-6 shadow-sm">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
        
        <div className="shrink-0">
          <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-full overflow-hidden bg-muted border-4 border-border">
            {avatarUrl ? (<img src={avatarUrl} alt={username} className="h-full w-full object-cover"/>) : (<div className="h-full w-full flex items-center justify-center bg-primary/10">
