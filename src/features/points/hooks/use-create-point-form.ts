...
import { getInitialGeolocation, useSharedUserLocation, updateSharedUserCoords, } from "@/shared/lib/user-location";
...
export function useCreatePointForm() {
    ...
    const { activeCrop, enqueueFiles, shiftQueue, clearQueue, } = usePointPhotoCropQueue();
    ...
    const handleCreateContainer = () => {
        if (!newContainerTitle.trim())
            return;
        if (!/^#([A-Fa-f0-9]{6})$/.test(newContainerColor))
            return;
    ...
    return {
        ...
        handleSubmit,
        isSubmitting: createPointMutation.isPending ||
