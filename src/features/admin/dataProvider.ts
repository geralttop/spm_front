...
export const dataProvider: DataProvider = {
    ...
    getOne: (resource, params) => httpClient(`${getApiUrl()}/admin/${resource}/${params.id}`).then(({ json }) => ({
        data: json,
    })),
    getMany: (resource, params) => {
        const promises = params.ids.map(id => httpClient(`${getApiUrl()}/admin/${resource}/${id}`));
    ...
    create: (resource, params) => httpClient(`${getApiUrl()}/admin/${resource}`, {
        method: 'POST',
        body: JSON.stringify(params.data),
    }).then(({ json }) => ({
        data: { ...params.data, id: json.id } as any,
    })),
    delete: (resource, params) => httpClient(`${getApiUrl()}/admin/${resource}/${params.id}`, {
        method: 'DELETE',
