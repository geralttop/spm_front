...
export default function SearchPage() {
    ...
    const { data: searchResults = [], isLoading: isSearching } = useSearchUsersQuery(debouncedQuery, hasSearched);
    ...
    return (<div className="min-h-[100dvh] bg-background py-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(0.75rem,env(safe-area-inset-top))] sm:py-6 lg:py-8">
      <div className="container mx-auto max-w-2xl px-[max(0.75rem,env(safe-area-inset-left))] pr-[max(0.75rem,env(safe-area-inset-right))] sm:px-6 lg:px-8">
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="relative">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-stretch sm:gap-2">
              <div className="relative min-w-0 flex-1">
                <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-text-muted sm:h-5 sm:w-5" aria-hidden/>
                <Input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={handleKeyDown} placeholder={t("search.placeholder")} autoComplete="off" className="touch-target pl-10 pr-10 text-base sm:text-sm"/>
                {searchQuery && (<button type="button" onClick={handleClearSearch} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md p-2 text-text-muted transition-colors hover:bg-accent hover:text-text-main touch-target" aria-label={t("search.clear")}>
                    <X className="h-4 w-4 sm:h-5 sm:w-5"/>
                  </button>)}
              </div>
              <Button type="button" onClick={handleSearch} disabled={!searchQuery.trim() || isSearching} className="w-full shrink-0 touch-target sm:w-auto sm:min-w-[7rem]">
          ...
          <div className="space-y-4">
            {isSearching && (<div className="py-6 text-center text-text-muted sm:py-8">
                <div className="animate-pulse text-sm sm:text-base">{t("search.searching")}</div>
              </div>)}

            {!isSearching && hasSearched && filteredResults.length === 0 && (<div className="py-8 text-center text-text-muted sm:py-12">
            ...
            {!isSearching && filteredResults.length > 0 && (<div className="space-y-3 sm:space-y-4">
            ...
            {!hasSearched && (<div className="py-10 text-center text-text-muted sm:py-16">
