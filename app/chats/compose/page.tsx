...
function ChatComposeInner() {
    ...
    const handleSend = useCallback((text: string) => {
        if (!validPeer)
            return;
        sendMutation.mutate({ peerUserId: peerId, text }, {
            onSuccess: (data) => {
    ...
    if (!validPeer) {
        return (<div className="mx-auto w-full max-w-3xl py-8 text-center text-text-muted md:max-w-4xl">
        <p>{t("common.error")}</p>
    ...
    if (loadError || !peer) {
        return (<div className="mx-auto w-full max-w-3xl py-8 text-center md:max-w-4xl">
