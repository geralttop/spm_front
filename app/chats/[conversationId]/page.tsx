...
import {
  ...
  useChatConversationMetaQuery,
  useChatMessagesInfiniteQuery,
  useSendChatMessageMutation,
} from "@/shared/lib/hooks/queries/use-chats-queries";
...
export default function ChatConversationPage() {
  ...
  const metaQuery = useChatConversationMetaQuery(
  ...
  const onWs = useCallback(
    (event: string, data: unknown) => {
      if (event !== "message:new" || !data || typeof data !== "object") return;
      const d = data as {
      ...
      void (async () => {
        ...
        if (fromPeer) {
          try {
            ...
            /* сеть / 401 — инвалидация всё равно подтянет актуальное состояние */
          }
        }
  ...
  if (!enabled) {
    return (
      <div className="w-full py-8 text-center text-text-muted">
        {t("common.error")}
      </div>
    );
  }
  ...
  if (metaQuery.isError || !metaQuery.data) {
    return (
      <div className="mx-auto w-full max-w-3xl py-8 text-center md:max-w-4xl xl:max-w-5xl">
  ...
  return (
    <div className="mx-auto flex h-[calc(100dvh-5rem)] max-h-[calc(100dvh-5rem)] w-full min-h-0 max-w-3xl flex-col overflow-hidden sm:h-[calc(100dvh-7rem)] sm:max-h-[calc(100dvh-7rem)] md:max-w-4xl xl:max-w-5xl 2xl:max-w-6xl">
      ...
      <div
        ref={scrollRef}
        className="min-h-0 flex-1 space-y-2 overflow-y-auto overscroll-contain rounded-xl border border-border bg-muted/20 p-3 sm:p-4"
      >
        ...
        {messagesQuery.isLoading ? (
          ...
          messages.map((m) => {
            ...
            return (
              ...
              >
                <div
                  className={`max-w-[min(92%,36rem)] rounded-2xl px-3 py-2 text-sm sm:max-w-[min(78%,40rem)] md:max-w-[min(72%,44rem)] lg:text-base ${
                    mine
                ...
                >
                  ...
                  <time
                    className={`mt-1 block text-[10px] opacity-70 ${mine ? "text-right" : ""}`}
