...
export default function ManagePage() {
  ...
  const { activeTab, setActiveTab, points, categories, containers, setCategories, setContainers } =
    useManageStore();
  ...
  // Категории и контейнеры подгружаются в дочерних вкладках при первом открытии;
  // без этой загрузки счётчики на табах остаются 0, пока вкладку не откроют.
  useEffect(() => {
      ...
      <div className="max-w-4xl mx-auto px-1 sm:px-4 py-4 sm:py-8 lg:px-6">
        ...
        <div className="flex overflow-x-auto gap-1 sm:gap-2 mb-4 sm:mb-6 border-b border-border -mx-1 px-1 sm:-mx-0 sm:px-0 scrollbar-hide">
          <button
            ...
            className={`flex items-center gap-1.5 whitespace-nowrap px-3 sm:px-4 py-2 text-sm sm:text-base font-medium transition-colors shrink-0 ${
              activeTab === 'points'
