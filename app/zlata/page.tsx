...
import {
  useZlataAssetsQuery,
  useZlataTagsQuery,
  useUploadZlataAssetMutation,
  useAttachZlataTagMutation,
  useDetachZlataTagMutation,
  useDeleteZlataAssetMutation,
  useDeleteZlataTagMutation,
  useCreateZlataTagMutation,
} from "@/shared/lib/hooks/queries/use-zlata-queries";
    ...
    <div className="min-h-screen bg-background text-text-main">
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex flex-col gap-3 rounded-xl border border-border bg-surface p-4 dark:border-border dark:bg-surface">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <Input
              ...
              placeholder={t("zlata.filterPlaceholder")}
        ...
        <div className="mb-8 rounded-xl border border-border bg-surface p-4 dark:border-border dark:bg-surface">
          ...
          <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-end">
            <div className="flex-1 sm:max-w-xs">
              <Input
                ...
                placeholder={t("zlata.addTagPlaceholder")}
              />
            </div>
            <Button
              type="button"
              variant="secondary"
              disabled={!newCatalogTag.trim() || createTagMutation.isPending}
              onClick={() => {
          ...
          {allTags.length === 0 ? (
            <p className="text-sm text-text-muted">{t("zlata.tagsEmpty")}</p>
          ) : (
            <ul className="flex flex-wrap gap-2">
              {allTags.map((tag) => (
                <li
                  key={tag.id}
                  className="inline-flex items-center gap-1 rounded-full border border-border bg-background px-2 py-1 text-sm text-text-main dark:border-border dark:bg-background"
                >
                  <span>{tag.name}</span>
                  <button
                    type="button"
                    className="rounded p-0.5 text-text-muted hover:bg-accent hover:text-text-main"
                    aria-label={t("zlata.deleteTag")}
        ...
        {isLoading ? (
          ...
          <p className="py-12 text-center text-text-muted">{t("zlata.empty")}</p>
        ) : (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {assets.map((asset) => (
              <article
                key={asset.id}
                className="overflow-hidden rounded-xl border border-border bg-surface shadow-sm dark:border-border dark:bg-surface"
              >
                <div className="relative aspect-square w-full bg-background dark:bg-background">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={zlataImageUrl(asset.url)}
                    alt=""
                ...
                <div className="space-y-3 p-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {asset.tags.map((tag) => (
                      <span
                        key={tag.id}
                        className="inline-flex items-center gap-1 rounded-full bg-accent px-2 py-0.5 text-xs text-text-main"
                      >
                        {tag.name}
                        <button
                          type="button"
                          className="rounded p-0.5 hover:bg-background/50"
                          aria-label={t("zlata.detachTag")}
                          onClick={() =>
                            detachMutation.mutate({ assetId: asset.id, tagId: tag.id })
                          }
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      onChange={(e) =>
                        setTagInputs((prev) => ({ ...prev, [asset.id]: e.target.value }))
                      }
                      placeholder={t("zlata.addTagPlaceholder")}
                    ...
                    <Button
                      ...
                      onClick={() => addTagToAsset(asset.id)}
                    >
                      {t("zlata.addTag")}
                    </Button>
                  </div>
