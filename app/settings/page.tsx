...
const QUALITY_DOT_CLASS: Record<MapStyleQualityLevel, string> = {
    good: "bg-emerald-500 ring-2 ring-emerald-500/30 dark:bg-emerald-400 dark:ring-emerald-400/25",
    medium: "bg-amber-500 ring-2 ring-amber-500/30 dark:bg-amber-400 dark:ring-amber-400/25",
    poor: "bg-red-500 ring-2 ring-red-500/30 dark:bg-red-400 dark:ring-red-400/25",
};
const QUALITY_ARIA_KEY: Record<MapStyleQualityLevel, "badgeGood" | "badgeMedium" | "badgePoor"> = {
    good: "badgeGood",
    medium: "badgeMedium",
    poor: "badgePoor",
};
...
export default function SettingsPage() {
    ...
    const { availableMapStyles, defaultMapStyle, pointCardInitialView, isLoading: settingsLoading, loadSettings, toggleMapStyle, setDefaultMapStyle, setPointCardInitialView, resetToDefaults, } = useSettingsStore();
    const { sidebarOrder, loadSidebarOrder, setSidebarOrder: updateSidebarOrder, resetToDefaults: resetSidebarOrder, } = useSidebarStore();
    ...
    return (<div className="min-h-screen bg-background py-4 sm:py-8 pb-[max(1rem,env(safe-area-inset-bottom))]">
      <div className="container mx-auto max-w-4xl px-1 sm:px-4 lg:px-6">
        <div className="space-y-4 sm:space-y-6">
          
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
            <Button variant="outline" size="sm" onClick={() => router.back()} className="gap-2 w-full shrink-0 sm:w-auto touch-target">
          ...
          <div className="rounded-lg border border-border bg-card p-3 sm:p-6 shadow-sm">
            <div className="flex flex-col gap-2 xs:flex-row xs:items-center xs:justify-between mb-4 sm:mb-6">
              ...
              <Button variant="outline" size="sm" onClick={handleResetSidebarOrder} className="gap-2 self-start xs:self-auto shrink-0">
            ...
            return (<div key={itemId} draggable onDragStart={(e) => handleDragStart(e, index)} onDragOver={(e) => handleDragOver(e, index)} onDragLeave={handleDragLeave} onDrop={(e) => handleDrop(e, index)} onDragEnd={handleDragEnd} className={`flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-lg border bg-card cursor-move transition-all ${isDragging ? 'opacity-50 scale-95' : ''} ${isDragOver ? 'border-primary scale-105' : 'border-border'} hover:border-primary/50`}>
          ...
          <div className="rounded-lg border border-border bg-card p-3 sm:p-6 shadow-sm">
            ...
            return (<button key={paletteId} type="button" onClick={() => setColorPalette(paletteId)} className={`text-left relative p-2.5 sm:p-3 rounded-lg border transition-all w-full ${isSelected
                    ? "border-primary bg-primary/5 hover:bg-primary/10"
                    : "border-border bg-muted/30 hover:bg-muted/50"}`}>
                    <div className="flex items-start gap-2.5 sm:gap-3">
                      <div className={`mt-0.5 w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${isSelected
                    ? "border-primary bg-primary"
                    : "border-border bg-background"}`}>
                        {isSelected && (<Check className="h-3 w-3 text-primary-foreground"/>)}
                      </div>
                      <div className="flex gap-2 flex-1 min-w-0">
                        <div className="shrink-0 w-10 h-10 rounded-md border border-border overflow-hidden flex flex-col shadow-sm" aria-hidden>
                          <div className="h-1/2 w-full" style={{ backgroundColor: preview.background }}/>
                          <div className="h-1/2 w-full" style={{ backgroundColor: preview.primary }}/>
                        </div>
                        <div className="min-w-0 flex-1">
          ...
          <div className="rounded-lg border border-border bg-card p-3 sm:p-6 shadow-sm">
            <div className="flex flex-col gap-2 xs:flex-row xs:items-center xs:justify-between mb-4 sm:mb-6">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="group relative">
                    <button type="button" className="rounded p-0.5 text-text-muted hover:text-text-main focus:outline-none focus-visible:ring-2 focus-visible:ring-ring touch-target" aria-label={t("settings.mapStyleQuality.helpAria")}>
              ...
              <Button variant="outline" size="sm" onClick={handleReset} className="gap-2 self-start xs:self-auto shrink-0">
                <RotateCcw className="h-4 w-4"/>
                {t('settings.reset')}
              </Button>
            </div>
            ...
            return (<div key={key} className={`min-h-11 min-w-0 p-2 sm:p-2.5 rounded-lg border transition-all cursor-pointer ${isEnabled
                    ? 'border-primary bg-primary/5 hover:bg-primary/10'
                    : 'border-border bg-muted/30 hover:bg-muted/50'}`} onClick={() => canDisable && !settingsLoading && handleToggleStyle(styleKey)}>
                    <div className="flex min-w-0 items-center gap-2 sm:gap-2.5">
                      <div className={`h-5 w-5 shrink-0 rounded border-2 flex items-center justify-center transition-colors ${isEnabled
                    ? 'border-primary bg-primary'
                    : 'border-border bg-background'} ${!canDisable || settingsLoading ? 'opacity-50' : ''}`}>
                        {isEnabled && <Check className="h-3 w-3 text-primary-foreground"/>}
                      </div>

                      <h3 className="min-w-0 flex-1 truncate text-sm font-medium text-text-main" title={styleName}>
                        {styleName}
                      </h3>

                      <span className="inline-flex shrink-0 items-center justify-center" aria-label={qualityHint} title={qualityHint}>
                        <span className={`size-2.5 rounded-full ${QUALITY_DOT_CLASS[quality]}`} aria-hidden/>
                      </span>

                      {isDefault && (<span className="shrink-0 whitespace-nowrap text-[10px] px-1.5 py-0.5 rounded-full bg-primary/20 text-primary font-medium">
                          {t('settings.default')}
                        </span>)}

                      {isEnabled && !isDefault && (<button type="button" onClick={(e) => {
                        e.stopPropagation();
                        handleSetDefault(styleKey);
                    }} disabled={settingsLoading} className="shrink-0 whitespace-nowrap rounded border border-border bg-background px-1.5 py-1 text-[10px] font-medium text-text-main hover:bg-accent transition-colors disabled:opacity-50 min-h-9 sm:min-h-0 sm:px-2 sm:py-0.5 touch-target">
          ...
          <div className="rounded-lg border border-border bg-card p-3 sm:p-6 shadow-sm">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              <button type="button" onClick={() => void handlePointCardInitialView("map")} disabled={settingsLoading} className={`flex min-h-11 flex-1 items-center justify-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium transition-colors sm:min-h-0 sm:flex-none sm:px-6 ${pointCardInitialView === "map"
            ...
            : "border-border bg-background text-text-main hover:bg-accent"}`}>
              ...
              <button type="button" onClick={() => void handlePointCardInitialView("photos")} disabled={settingsLoading} className={`flex min-h-11 flex-1 items-center justify-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium transition-colors sm:min-h-0 sm:flex-none sm:px-6 ${pointCardInitialView === "photos"
            ? "border-primary bg-primary/10 text-text-main"
            : "border-border bg-background text-text-main hover:bg-accent"}`}>
