...
import { Map as MapComponent, MapControls, MapMarker, MapPopupPhotos, MarkerContent, MarkerLabel, MarkerPopup, MarkerTooltip, useMap, } from "@/shared/ui/map";
import { buttonVariants } from "@/shared/ui/button/button";
import { MAP_STYLES, type MapStyleKey } from "@/shared/config/map-styles";
import { MapPin, User, Calendar, Loader2, ChevronDown, MessageCircle, ExternalLink, } from "lucide-react";
...
function MapFlyToPointFromQuery({ pointId, allPoints, }: {
    pointId: string | null;
    allPoints: FeedPoint[];
}) {
...
function MapPageClient() {
    ...
    return (<div className="fixed inset-0 top-14 z-0 h-[calc(100dvh-3.5rem)] w-full overflow-hidden pb-[env(safe-area-inset-bottom)] sm:top-16 sm:h-[calc(100dvh-4rem)] lg:left-64 lg:w-[calc(100%-16rem)]">
      {loading || !mapStyleReady ? (<div className="flex h-full w-full flex-col items-center justify-center gap-3 bg-background">
          ...
          <div className="pointer-events-none absolute right-2 top-2 z-10 sm:right-4 sm:top-4">
            <div className="pointer-events-auto relative max-w-[min(13rem,calc(100vw-4.5rem))] rounded-lg border border-border bg-card/95 shadow-md backdrop-blur-sm sm:max-w-[15rem]">
              <label htmlFor="map-style-select" className="sr-only">
                {t("map.mapStyle")}
              </label>
              <select id="map-style-select" value={mapStyle!} onChange={(e) => setMapStyle(e.target.value as MapStyleKey)} className="w-full min-w-0 cursor-pointer appearance-none rounded-lg border-0 bg-transparent py-2 pl-2.5 pr-9 text-xs text-text-main focus:outline-none focus:ring-2 focus:ring-inset focus:ring-primary sm:py-2.5 sm:pl-3 sm:pr-10 sm:text-sm touch-manipulation" title={t("map.mapStyle")}>
                {availableMapStyles.map((key) => (<option key={key} value={key}>
                    {t(`mapStyles.${key}.name`)}
                  </option>))}
              </select>
              <ChevronDown className="pointer-events-none absolute right-2 top-1/2 h-4 w-4 -translate-y-1/2 text-text-muted sm:right-2.5" aria-hidden/>
            </div>
          </div>

          
          {!error && (<div className="pointer-events-none absolute left-2 top-2 z-10 sm:left-4 sm:top-4">
          ...
          <MapComponent center={center} zoom={zoom} styles={{
                light: MAP_STYLES[mapStyle!].light,
                dark: MAP_STYLES[mapStyle!].dark,
            }}>
            <MapFlyToPointFromQuery pointId={pointFromQuery} allPoints={allPoints}/>
            {points.map((point) => {
                ...
                const avatarUrl = resolveAuthorAvatarUrl(point.author.avatar ?? undefined);
                const authorInitials = getAuthorInitials(point);
                return (<MapMarker key={point.id} longitude={lng} latitude={lat}>
                  <MarkerContent>
                    <div className="relative size-9 cursor-pointer rounded-full p-[3px] shadow-lg hover:scale-110 transition-transform motion-reduce:transition-none" style={{
                      ...
                      <div className="flex size-full items-center justify-center rounded-full bg-background p-[1px]">
                        <div className="relative flex size-full items-center justify-center overflow-hidden rounded-full bg-muted text-[10px] font-semibold text-text-main">
                          {avatarUrl ? (<img src={avatarUrl} alt={`${point.author.firstName} ${point.author.lastName}`} className="h-full w-full object-cover"/>) : (<span>{authorInitials}</span>)}
                        </div>
                      </div>
                    </div>
                    <MarkerLabel position="bottom" className="max-w-[9rem] truncate rounded-md border border-border bg-card/95 px-1.5 py-0.5 text-[10px] font-medium text-text-main shadow-sm backdrop-blur-sm">
                      {markerLabelText(point)}
                    </MarkerLabel>
                  </MarkerContent>
                  <MarkerPopup closeButton className="p-0 w-[min(calc(100vw-2rem),17.5rem)] overflow-hidden border-border" initialOpen={pointFromQuery === point.id} onOpenChange={(open) => setOpenPostPointId(open ? point.id : null)}>
                    <MapPopupPhotos photos={point.photos ?? []} pointName={point.name} categoryColor={point.category?.color}/>
                    <div className="space-y-2 p-3">
                      <div>
                        {categoryName ? (<span className="text-xs font-medium uppercase tracking-wide text-text-muted">
                            {categoryName}
                          </span>) : (<span className="text-xs font-medium uppercase tracking-wide text-text-muted">
                      ...
                      {point.description ? (<p className="line-clamp-2 text-sm text-text-muted">{point.description}</p>) : null}

                      {commentsCount > 0 ? (<div className="flex items-center gap-1.5 text-sm text-text-muted">
                      ...
                      {point.address ? (<div className="flex items-start gap-2 text-xs text-text-muted">
                      ...
                      <div className="flex gap-2 pt-1">
                        <div className="flex min-w-0 flex-1 gap-1">
                          <a href={yandexMapsPointUrl(lat, lng)} target="_blank" rel="noopener noreferrer" className="inline-flex h-8 min-w-0 flex-1 items-center justify-center rounded-md bg-[#FC3F1D] text-sm font-semibold text-white transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FC3F1D] focus-visible:ring-offset-2 focus-visible:ring-offset-background" title={t("map.popup.openInYandex")} aria-label={t("map.popup.openInYandex")}>
                            <span className="shrink-0" aria-hidden>
                              Я
                            </span>
                          </a>
                          <a href={googleMapsPointUrl(lat, lng)} target="_blank" rel="noopener noreferrer" className="inline-flex h-8 min-w-0 flex-1 items-center justify-center rounded-md bg-[#34A853] text-sm font-semibold text-white transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#34A853] focus-visible:ring-offset-2 focus-visible:ring-offset-background" title={t("map.popup.openInGoogle")} aria-label={t("map.popup.openInGoogle")}>
                            <span className="shrink-0" aria-hidden>
                              G
                            </span>
                          </a>
                        ...
                        : `/points/${point.id}`} target="_blank" rel="noopener noreferrer" className={cn(buttonVariants({ size: "sm", variant: "outline" }), "size-8 shrink-0 p-0")} aria-label={t("map.popup.openInNewTab")}>
            ...
            {userLocation && (<MapMarker longitude={userLocation.longitude} latitude={userLocation.latitude} anchor="center">
                <MarkerContent className="cursor-default">
                  <div className="relative flex size-7 items-center justify-center" role="img" aria-label={t("map.yourLocation")}>
                    <span className="absolute inset-0 rounded-full bg-primary/20 motion-safe:animate-pulse" aria-hidden/>
                    <span className="relative size-3.5 rounded-full border-2 border-background bg-primary shadow-md ring-2 ring-primary/50" aria-hidden/>
                  </div>
                </MarkerContent>
                <MarkerTooltip>{t("map.yourLocation")}</MarkerTooltip>
              </MapMarker>)}

            <MapControls position="bottom-right" showZoom showCompass showLocate showCenterOnPoint centerOnPointCoords={centerOnPointCoords} showFullscreen onLocate={(coords) => setUserLocation({
          ...
          {!error &&
                ...
                !loading &&
                mapStyleReady &&
                points.length === 0 && (<div className="pointer-events-none absolute left-1/2 top-20 z-10 max-w-[min(20rem,calc(100%-2rem))] -translate-x-1/2 rounded-lg border border-border bg-card/95 px-3 py-2 text-center shadow-md backdrop-blur-sm sm:top-24">
                <p className="text-xs text-text-muted sm:text-sm">
                  {t("map.containerMapEmpty")}
                </p>
              </div>)}

          {error && (<div className="absolute left-1/2 top-16 z-20 max-w-[calc(100%-1rem)] -translate-x-1/2 rounded-lg border border-destructive/30 bg-destructive/10 p-3 shadow-lg sm:top-20 sm:max-w-lg sm:p-4">
...
export default function MapPage() {
    const { t } = useTranslation();
    return (<Suspense fallback={<div className="fixed inset-0 top-14 z-0 flex h-[calc(100dvh-3.5rem)] w-full flex-col items-center justify-center gap-3 bg-background pb-[env(safe-area-inset-bottom)] sm:top-16 sm:h-[calc(100dvh-4rem)] lg:left-64 lg:w-[calc(100%-16rem)]">
          <Loader2 className="h-8 w-8 animate-spin text-primary"/>
          <p className="text-sm text-text-muted">{t("map.loadingPoints")}</p>
        </div>}>
      <MapPageClient />
